...
describe('useVaccinationsStore', () => {
  it('garde la liste du dernier animal demandé quand la réponse du précédent arrive après', async () => {
    repository.seed(rage(LUNA))
    let finishMilo: (items: Vaccination[]) => void = () => {}
    ...
    expect(store.vaccinations).toHaveLength(1)
    expect(store.vaccinations[0]?.animalId).toBe(LUNA)
  })
  ...
  it('ignore l’échec d’un chargement dépassé par celui d’un autre animal', async () => {
    ...
    repository.listByAnimal.mockReturnValueOnce(
      new Promise<Vaccination[]>((_resolve, reject) => {
        failMilo = reject
      }),
    )
    const store = useVaccinationsStore()
  ...
  it('transmet la création aux statistiques, avec l’espèce de l’animal', async () => {
    ...
    expect(track).toHaveBeenCalledExactlyOnceWith('vaccination_created', { species: 'dog' })
  })
