...
| **Stockage partagé (export enregistré, #343)** | `@capacitor/filesystem` 8.1.3, README et code du plugin (consultés le 23/09/2026) : `Directory.Documents` « only allows to access the files/folders created by your app on Android 11 and newer » ; « in Android 10 and older, this API requires » `READ_EXTERNAL_STORAGE` et `WRITE_EXTERNAL_STORAGE` ; sur Android 10, Documents n'est accessible qu'avec `android:requestLegacyExternalStorage="true"`. Ces deux permissions ne figurent pas parmi les formulaires de déclaration de la ligne ci-dessus ; « All files access » (`MANAGE_EXTERNAL_STORAGE`), si. | Déclarer `READ/WRITE_EXTERNAL_STORAGE` avec `android:maxSdkVersion="29"` : aucune permission sur Android 11 et plus. Demande en contexte, au premier enregistrement d'un export, jamais au lancement. Jamais `MANAGE_EXTERNAL_STORAGE`. `pnpm test:manifest` vérifie le `maxSdkVersion` dans le manifest fusionné. Détail : `docs/technical/export-format.md`. |
...
- Retrait : `opt_out_capturing()` puis `reset(true)`, et l'app efface elle-même les clés `ph_*` / `__ph_*` du `localStorage` du WebView (`distinct_id` compris), que le SDK ait été chargé ou non.
...
9. **Version de Play Billing Library embarquée par le plugin Capacitor** : vérifié le 2026-09-15 au build Gradle, `@revenuecat/purchases-capacitor` 13.5.1 embarque Play Billing Library 8.3.0 (exigence PBL ≥ 7 au 31/08/2026 satisfaite). À recontrôler à chaque montée du plugin.
