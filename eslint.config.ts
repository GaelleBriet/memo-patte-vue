...
const DYNAMIC_I18N_KEYS = [
  ...
  '/^treatments\\.form\\.errors\\.(name|type|frequency|frequencyMax|lastDoseDate|lastDoseDateFuture|nextDueDate|nextDueDateBeforeLastDose)$/',
  '/^weight\\.form\\.errors\\.(animalId|weightKg|weightKgMax|measuredOn|measuredOnFuture)$/',
  '/^weight\\.(unit|unitName)\\.(kg|lb)$/',
  '/^settings\\.data\\.weightUnit\\.spoken\\.(kg|lb)$/',
  ...
  '/^home\\.reminder\\.(vaccination|deworming|antiparasitic)$/',
  '/^home\\.row\\.spokenType\\.(vaccination|deworming|antiparasitic)$/',
  '/^home\\.row\\.due\\.(overdue|today|tomorrow)$/',
  '/^vaccinations\\.sheet\\.done\\.choices\\.(oneYear|threeYears|otherDate|none)$/',
  '/^home\\.due\\.(overdue|today|tomorrow|later)$/',
  '/^plus\\.headline\\.(pdf|general)$/',
